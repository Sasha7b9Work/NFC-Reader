...
/* Validate parameters
   ...
   /* Validate Card Key Number */
    ...
    (void)memcpy(&bCmdBuff[bRndLen + 1], &bRndB[1], bRndLen - 1);
    ...
    uint8_t ** ppRxdata, uint16_t * pRxdataLen)
{
        ...
        (bCmdBuff[0] != PHAL_MFDFLIGHT_RESP_ADDITIONAL_FRAME))
    {
        ...
        if (wCmdLen > 1)
        {
            (void)memcpy(&bWorkBuffer[wWorkBufferLen], &bCmdBuff[1], (wCmdLen - 1));
    ...
    if (pDataParams->bAuthMode == PHAL_MFDFLIGHT_AUTHENTICATEEV2)
    {
        ...
        if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
        {
            (void)memcpy(bCMacCard, &pRecv[wRxlen - 8], 8);
            wRxlen -= 8;

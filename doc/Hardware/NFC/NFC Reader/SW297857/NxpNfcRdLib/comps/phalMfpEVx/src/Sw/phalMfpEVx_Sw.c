    ...
    uint16_t wKeyVersion, uint8_t * pUid, uint8_t bUidLength)
{
    ...
    {
        return PH_ADD_COMPCODE(PH_ERR_USE_CONDITION, PH_COMP_AL_MFPEVX);
    }
...
phStatus_t phalMfpEVx_Sw_ReadSign(phalMfpEVx_Sw_DataParams_t * pDataParams, uint8_t bLayer4Comm, uint8_t bAddr, uint8_t ** pSignature)
{
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phalMfpEVx_Int_ComputeErrorResponse(wRxLength, pResponse[0], bLayer4Comm));
...
phStatus_t phalMfpEVx_Sw_ResetAuth(phalMfpEVx_Sw_DataParams_t * pDataParams)
{
    ...
    /* Perform ResetAuth. */
    PH_CHECK_SUCCESS_FCT(statusTmp, phalMfpEVx_Int_ResetAuth(pDataParams->pPalMifareDataParams, pDataParams->bWrappedMode, pDataParams->bExtendedLenApdu));
    ...
    {
        /* Reset VCA session */
        PH_CHECK_SUCCESS_FCT(statusTmp, phalVca_SetSessionKeyUtility((phalVca_Sw_DataParams_t *)pDataParams->pVCADataParams,

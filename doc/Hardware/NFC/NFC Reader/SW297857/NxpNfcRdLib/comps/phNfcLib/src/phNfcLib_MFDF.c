...
static phStatus_t phNfcLib_MFDF_Exchange_Cmd(uint8_t bWrappedMode,  uint8_t * pSendBuff, uint16_t wCmdLen,  uint8_t ** ppResponse, uint16_t * pRxlen)
{
    ...
    if (0U != (bWrappedMode))
    {
        pApdu[1] = pSendBuff[0]; /* MIFARE DESFire cmd code in INS */
                          ...
                          )
{
    ...
    PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_GetConfig(&gphNfcLib_Params.sHal,
    ...
    if (0U != (gphNfcLib_State.bWrappedMode))
    {
        ...
        pApdu[1] = pSendBuff[0];  /* MIFARE DESFire command code. */
        /* Encode APDU Length*/
    ...
    *pResponse = pRecv;
    ...
    while (bStatusByte == PH_NFCLIB_MFDF_RESP_ADDITIONAL_FRAME)
    {
        ...
        PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SetConfig(&gphNfcLib_Params.sHal,
            PHHAL_HW_CONFIG_RXBUFFER_STARTPOS,
            wNextPos
            ));
